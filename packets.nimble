...
task test, "tests":
  ...
  exec "nim c -r -d:packetDumpTree --nimcache=./.nimcache/ tests/packets.nim"
  exec "nim c -r tests/packets.nim"
  echo "Running no IDs stdlib json tests"
  exec "nim c -r -d:packetDumpTree -d:disablePacketIDs --nimcache=./.nimcache/ tests/packets.nim"
  
task bench, "benchmarks":
  echo "Running stdlib benchmark"
  exec "nim c -r -d:release -d:disablePacketIDs --nimcache=./.nimcache/ tests/benchmark.nim"
