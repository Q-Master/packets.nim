...
proc load*[T: SomeSignedInt](ctx: var TPacketDataSource, dest: var T) =
...
proc load*[T: SomeUnsignedInt](ctx: var TPacketDataSource, dest: var T) =
  if ctx.toCtx.parser.tok != tkInt:
    raise newException(ValueError, "Wrong field type: " & $ctx.toCtx.parser.tok)
  let decoded = parseBiggestInt(ctx.toCtx.parser.a)
  if decoded > high(T):
...
proc dump*[T: SomeSignedInt | SomeUnsignedInt | float | float32 | float64](t: T, dest: var string) =
  dest.add($t)
